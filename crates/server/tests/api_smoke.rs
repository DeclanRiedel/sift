...
#[tokio::test]
async fn ddl_generation_table_preserves_native_definition() {
    use sift_protocol::{ObjectKind, ObjectPath};
    let definition = r#"CREATE TABLE "public"."users" ("id" bigint GENERATED BY DEFAULT AS IDENTITY (START WITH 17), "name" varchar(31) COLLATE "C");"#;
    let driver = MockDriver::builder()
        .engine(Engine::Postgres)
        .execute_ok(vec![
            Page::NextResult {
            ...
            Page::Rows {
                rows: vec![Row::new(vec![Value::Text(definition.into())])],
            },
            Page::Done {
                affected_rows: None,
                warnings: vec![],
