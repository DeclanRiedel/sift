        ...
        self.modal.as_ref().map(|modal| {
            ...
            let content = match modal {
                ...
                Modal::RoomAdministration => {
                    ...
                    div().w_full().max_h(px(620.)).flex().flex_col().gap_3()
                        .child(div().flex().items_center().justify_between().child(div().debug_selector(|| "room-administration-heading".into()).font_weight(gpui::FontWeight::SEMIBOLD).child("Room administration")).child(Button::new("refresh-room-administration", "Refresh").tone(ButtonTone::Ghost).loading(self.room_admin_pending).on_click(cx.listener(|shell, _, _, cx| shell.open_room_administration(cx)))))
