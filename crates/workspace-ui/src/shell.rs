...
impl Pane {
    ...
    fn finish_tab_bar_drag(
        ...
        if drag.pane_id == self.id {
            ...
            cx.emit(PaneEvent::MoveItemRequested {
                ...
                target: PaneDropTarget::Tab(insertion_index),
            });
        }
    ...
    fn finish_tab_drag(&mut self, drag: &TabDrag, _: &mut Window, cx: &mut Context<Self>) {
        ...
        } else if let PaneDropTarget::Tab(to) = target {
    ...
    /// Adopt a tab detached from another pane at a requested tab-bar slot.
    fn receive_item(
        &mut self,
...
impl WorkspaceShell {
    ...
    fn move_item_to_target(
        ...
        let collapse_before_split = source_pane != target_pane
            && source_pane.read(cx).items.is_empty()
            && matches!(
                target,
                PaneDropTarget::Above
                    | PaneDropTarget::Before
    ...
    fn move_item_to_root(
        ...
        if source_pane.read(cx).items.is_empty() && self.panes.len() == 1 {
            source_pane.update(cx, |pane, cx| pane.receive_item(transfer, None, cx));
            self.clear_tab_drag_previews(cx);
            self.focus_active_pane(window, cx);
            cx.notify();
            return;
        ...
        let direction = match target {
            ...
            PaneDropTarget::After => SplitDirection::Right,
            PaneDropTarget::Below => SplitDirection::Down,
            PaneDropTarget::Center | PaneDropTarget::Tab(_) => return,
        };
        let id = self.next_id;
        self.next_id += 1;
        let vim_mode = self.vim_mode_default();
        ...
        let insertion_index = if matches!(target, PaneDropTarget::Above | PaneDropTarget::Before) {
            0
        } else {
            self.panes.len()
        };
    ...
    /// Workspace-edge targets split the entire pane group. Leaf-pane targets
    /// remain available away from the outer edge for nested corner splits.
    fn track_root_tab_drag(
        &mut self,
        event: &gpui::DragMoveEvent<TabDrag>,
        _: &mut Window,
        cx: &mut Context<Self>,
            ...
            || !event.bounds.contains(&event.event.position)
        {
            ...
            let distances = [
                ...
                (height - relative_y, PaneDropTarget::Below),
                (relative_x, PaneDropTarget::Before),
            ];
            distances
                .into_iter()
...
impl gpui::Render for WorkspaceShell {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let root_tab_drop_preview = self.root_tab_drop_target.map(|target| {
            div()
                .debug_selector(move || match target {
                    PaneDropTarget::Above => "root-pane-drop-preview-above".into(),
                    PaneDropTarget::Before => "root-pane-drop-preview-before".into(),
                    PaneDropTarget::After => "root-pane-drop-preview-after".into(),
                    PaneDropTarget::Below => "root-pane-drop-preview-below".into(),
                    PaneDropTarget::Center | PaneDropTarget::Tab(_) => unreachable!(),
                })
                .absolute()
                .bg(pane_preview_tint)
                        ...
                        div()
                            ...
                            .child(
                                div()
                                    ...
                                    .min_w_0()
                                    .min_h_0()
                                    .on_drag_move::<TabDrag>(cx.listener(Self::track_root_tab_drag))
                                    .child(pane_elements)
                                    .children(root_tab_drop_preview),
                            )
...
mod tests {
    ...
    fn single_tab_edge_drag_replaces_its_source_pane(cx: &mut TestAppContext) {
        ...
        let preview = cx
            ...
            .expect("Zed-style local split preview");
        assert_eq!(preview.left(), pane.left());
        assert_eq!(preview.top(), pane.top());
        assert_eq!(preview.size.width, pane.size.width / 2.0);
        assert_eq!(preview.size.height, pane.size.height);
    ...
    #[gpui::test]
    fn sole_tab_vertical_move_previews_the_post_collapse_region(cx: &mut TestAppContext) {
        let mut state = PresentationState::default();
        state.workspace.panes[0].items.push(ItemPresentation {
            id: 2,
            kind: ItemKind::Query,
        ...
        workspace.update_in(&mut cx, |shell, window, cx| {
            ...
            shell.move_item_to_target(source.clone(), 1, PaneDropTarget::Below, window, cx);
        });
        cx.run_until_parked();

        let source = cx.debug_bounds("tab-2").expect("upper sole tab");
        let target_id = workspace.read_with(&cx, |shell, cx| {
            shell
                .panes
                .iter()
                .find(|pane| pane.read(cx).contains_item(1))
                .expect("lower pane")
                .read(cx)
                .id
        });
        ...
        let drop = point(target.center().x, target.bottom() - px(10.));
        cx.simulate_mouse_move(drop, MouseButton::Left, Modifiers::default());
        cx.run_until_parked();

        let preview = cx
            .debug_bounds("root-pane-drop-preview-below")
    ...
    #[gpui::test]
    fn escape_cancels_tab_drag_without_leaving_a_drop_target(cx: &mut TestAppContext) {
        let mut state = PresentationState::default();
        state.workspace.panes[0].items.push(ItemPresentation {
            id: 2,
            kind: ItemKind::Query,
        ...
        let tab = cx.debug_bounds("tab-1").expect("drag source");
        let pane = cx.debug_bounds("pane-body-1").expect("pane body");
        let start = tab.center();
        cx.simulate_mouse_down(start, MouseButton::Left, Modifiers::default());
        cx.simulate_mouse_move(
            point(pane.left() + px(10.), pane.center().y),
            MouseButton::Left,
            Modifiers::default(),
        );
        cx.run_until_parked();
        ...
        workspace.read_with(&cx, |shell, cx| {
            assert_eq!(shell.panes.len(), 1);
            assert_eq!(shell.panes[0].read(cx).tab_drop_target, None);
        });
    }
    ...
    #[gpui::test]
    fn pane_tab_drag_edge_previews_and_creates_a_sibling_pane(cx: &mut TestAppContext) {
        let mut state = PresentationState::default();
        state.workspace.panes[0].items.push(ItemPresentation {
            id: 2,
            kind: ItemKind::Query,
        ...
        let drop = point(target.left() + px(12.), target.center().y);
        cx.simulate_mouse_move(drop, MouseButton::Left, Modifiers::default());
        cx.run_until_parked();
