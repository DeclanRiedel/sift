...
pub fn apply(engine: Engine, sql: &str, transform: &ResultTransform) -> Result<String, String> {
    ...
    if !transform.sorts.is_empty() {
        ...
        for (index, sort) in transform.sorts.iter().enumerate() {
            ...
            output.push_str(&quote_ident(&sort.column, engine));
...
fn predicate(engine: Engine, filter: &ResultFilter) -> Result<String, String> {
    ...
    let column = quote_ident(&filter.column, engine);
